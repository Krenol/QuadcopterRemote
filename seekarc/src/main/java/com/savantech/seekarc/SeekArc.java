...
public class SeekArc extends View {
    ...
    protected void onMeasure(int widthMeasureSpec,int heightMeasureSpec)
    {
        ...
        width = Math.min(tempWidth, tempHeight);
        height = Math.min(tempWidth, tempHeight);
    ...
    {
        this.sweepAngle = Math.min(sweepAngle, 360);;
        invalidate();
    }
    ...
    {
        isAntiClockwise = direction.toLowerCase().equals(ANTICLOCKWISE);
        invalidate();
    }
    ...
    boolean isTouchinRange(float progressAngle,float x,float y,boolean isMoving)
    {
        ...
        boolean boundary = isMoving ? radius >= (0.7 * minRadius + 10)
